const data = {
  type: "STACK",
  title: "Tips and Tricks",
  chapterId: 1,
  subChapters: [
    {
      id: 0,
      title: "Being a Manager",
      type: "STACK",
      content:
        ["It took 4 years for Chef Kasem to learn how to become an effective manager and through the process, reflect on his own life and future"],
      data: [
        {
          title: "First point: Timeline of enlightenment",
          data: [
            "1st year – selfish, high-tempered, fire many people in 1 day, beat people down",
            "2nd year – even worse, didn’t accept any mistakes, would send people home for a tiny mistake, like no lipstick",
            "3rd year - one of his coworkers said “you are a really selfish person, you are always negative and bad energy and trying to beat people down”, Pop stayed angry at it for a long time. Only cared about sales and growth of the business, not about the people",
            "4th year – he felt nauseous one day and realized he forgot to breathe, he realized that his coworker was right. He has changed into a different person, an unhappy person, and decided to change"
          ]
        },
        {
          title: "Keys to his growth",
          data: [
            "His coworker telling him that he is a negative, unhappy person",
            "He almost passed out from not being able to breath by himself",
            "Quitting the manager role at Osha Thai and being depressed for 2 months before working at Sweet Maple",
            "Reading The Secret, wishing for money to help him, and then all of a sudden getting a check in the mail for tax return"
          ]
        }
      ]
    },
    {
      title: "Personal life",
      content:
        "Chef Kasem met his wife early on in his career as a server. Though it hasn’t been easy working with your spouse, his wife, ___, served a significant role in being his support system, adviser, and partner throughout his journey.",
      data: [
        {
          title: "Meeting his Wife",
          data: [
            "Met his wife at Grand Pubar",
            "Always positive thinking",
            "Strong character",
            "Very independent",
            "Were friends for a couple of years, go out drinking, fun",
            "Proposed to his wife while he was a manager at Osha Thai"
          ]
        },
        {
          title: "Tips on managing a personal life",
          data: [
            "Keep everything at work and don’t bring it home",
            "If you just want money and focus on that, you will be so stressed that you have no personal life",
            "Share your feelings",
            "Many designs and ideas are from Ing, she is very fashionable",
            "First child 37 years old, daughter",
            "Spend two hours a day with daughter even though he has to work"
          ]
        },
        {
          title: "Realizations",
          data: [
            "Accept your mistakes, deeply, otherwise you cannot improve",
            "Practice your skill, understand the bigger picture of what you’re doing",
            "Never run away from your challenges",
            "Time never stops, you cannot ask for another 10 minutes in a day. You have to make a decision right now",
            "Train your mindset through reading books and listening to podcasts",
            "See opportunity in everything",
            "Having the right expectations of others, training them properly versus expecting them to be perfect",
            "Have the right communication as a leader",
            "Think positive",
            "Create a handbook, manual, or system to run your business that is clear and effective",
            "Focus on something you’re already good at and make it even better (cuisine, background)",
            "Know exactly what you are looking for and build towards that",
            "A lot of money in a short amount of time?, Financial freedom of time for yourself and family?",
            "If you want happiness with profit then you need to be willing to spend money, hire more people, and build a system",
            "Don’t be superman",
            "Give yourself days off, appreciate others, delegate",
            "Separate work and life",
            "After you go home, just leave all the problems and stress at work"
          ]
        },
        {
          title: "Key Realizations",
          description:
            "After 18 years of being in the restaurant journey and numerous ups and downs, Chef Kasem notes down his key realizations of what is most important in building a successful food business",
          data: [
            {
              title: "Points on how to run a successful food business",
              data: [
                "His managers become his partners (growth potential) – retention",
                "Pop train them on building a restaurant system",
                "Grow a server into a partner",
                "Long-lasting business rather than only lasting 4-20 years max as a restaurant",
                "Have enough employees that rotate so never short on employees",
                "Keep full time employees close and provide good bonuses (tax write offs) and environment",
                "Managing the employee time",
                "4 hours per shift, otherwise in danger of lack of break time if people go over and work overtime",
                "Employee handbook",
                "Have signatures to protect yourself",
                "The greater the supply you order and more guarantee that you can provide, the more room for negotiation you have",
                "Setting the price, ordering on time",
                "Have cameras everywhere",
                "Have a system",
                "Build a great team",
                "Have all types of managers in different areas",
                "Quarterly training",
                "Annual server tasting",
                "After a few months, let the sytem work for you",
                "Focus on building a restaurant with a good environment and positive energy"
              ]
            },
            {
              title: "Examples of stories",
              data: [
                "Lady bought 12 shots of Black Label, when the employee stopped her and asked her for money, she slapped the employee",
                "Customer would come in with a suit and order a lot, then would say he has no money",
                "Just say: Thank you for trying to hard for this. Don’t try to argue with them.",
                "Lady accused coworkers of sexual harassment – lost case",
                "A few guys worked overtime and sued him for not giving them the proper break – dropped case",
                "A new employee got close to all the other employees and worked with a lawyer to try and sue Farmhouse for a bad environment and system for employees – won case",
                "Found out his wife worked at the labor office and he did this to many restaurants",
                "Be prepared for scam artists and make sure that you protect yourself with a handbook, signatures, and proper schedules of employees.",
                "Have cameras everywhere.",
                "PG&E scams, employee would pay over the phone for Farmhouse and get scammed",
                "Cleaning scams, come in and clean and ask for money",
                "Ask for specific license. Don’t be intimidated."
              ]
            },
            {
              title: "Today’s focus",
              data: [
                "Built by less than $1,000 in pocket",
                "Now today $6,000,000/year revenue for each restaurant",
                "Good credit for 12 years (can buy a building)",
                "A goal of happiness with profit",
                "Work-life balance, spend time with wife, daughter, and 2 dogs",
                "Take managers out to dinner all the time to maintain good relationship",
                "New project, breakfast in Tenderloin"
              ]
            }
          ]
        }
      ]
    }
  ]
};

export default data;
